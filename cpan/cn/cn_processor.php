...
*/
	$sourceFile = "../../data/cnlist.xml";
/*
	function setWriteLock(){
		$lconn = mysqli_connect('localhost', 'soenewsadmin', '12345678', 'soenews') or die ("cannot connect to db : ".mysqli_error());

		$ret = false;
		$timeout = 20;
		$count = 0;
		while(!$ret){
			$ret = mysqli_query($lconn, "select writeL, readL from openfiles where filename = 'cnlist.xml'");
			if($ret){
				$results = mysqli_fetch_assoc($ret);
				if($results['writeL'] === 'y'){
					$ret = false;
				}
				else if($results['writeL'] === 'n'){
					$ret = mysqli_query($lconn, "update openfiles set writeL='y' filename = 'cnlist.xml'");
					if(!$ret){
						echo mysqli_error($lconn);
					}
				}
			}
			if(++$count === $timeout and $ret === false){
				return false;
			}
		}
		return true;
...
*/
	if($_POST){
		...
		function makeXMLString($texts, $statuses, $pubdates, $sources){
			$count = count($texts);
			$retString = "<root>";
			for($z = 0; $z < $count; ++$z){
		...
		if(strcmp($action, "removeSingle") == 0){
			...
			$outputString = makeXMLString($texts, $statuses, $pubdates, $sources);
		...
		elseif(strcmp($action, "rearrange") == 0){
			...
			$outputString = makeXMLString($ttexts, $tstatuses, $tpubdates, $tsources);
