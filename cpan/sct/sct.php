<?php
	$pageTitle = "Scroll Text Interface";
	$curPath = "> Scroll Text Interface";
	$uname = "dovahkin";
	$level = "master";
...
<style>
	...
	#text_value, #text_representation{
		margin-left:10px;
		margin-right:10px;
	}
	#seperator_control_change_button{
		margin-left: 20px;
		margin-top:0;
		padding: 3px;
	}
...
<div id='seperator_control'>
	<div id='seperator_control_text'>Current Seperator : </div>
	<div id='text_value'>Value = </div>
	<div id='seperator_control_value'></div>
	<div id='text_representation'>Representation = </div>
	<div id='seperator_control_representation'></div>
	<button id='seperator_control_change_button' onclick='changeSeperator();'>change</button>
</div>
...
function changeSeperator(){
		...
		$("#inputbox_title").text("Change Seperator");
		$("#inputbox_single_input").val(seperator);
		$("#inputbox").slideDown(200);
		$("#inputbox_single_input").focus();
		inputMode = "changeSeperator";
}
	...
	if(multipleMode){
		...
		if(see_history_action == "block"){
			if(selected.length != 0){
				$.post('./sct_processor.php', {'uname':username, 'action' : 'blockMultiple', 'selection' : selected.toString()}, function(result){
					console.log("blockMultiple selection : " +selected + " : result : " + result);
					fetchDataAndPopulate('../../getScrolllist.php');
				});
			}
		}
			...
			if(selected.length != 0){
				...
				if(confirmation){
					$.post('./sct_processor.php', {'uname' : username, 'action' : 'removeMultiple', 'selection' : selected.toString()}, function (result){
			...
			if(ids.length != 0){
				$.post("./sct_processor.php", {"uname" : username, "action" : 'rearrange', 'newOrder' : ids.toString() }, function(result){
...
function editNews(index){
		$("button").attr('disabled', true);
		$("#inputbox_positive, #inputbox_negative").attr('disabled',false);
		$("#inputbox_title").text("Edit Scroll News");
...
function submitNews(){
	...
	if(newNews != ""){
		if(inputMode == "add"){
			$.post('./sct_processor.php', {'uname' : username, 'action' : 'addSingle', 'data' : newNews}, function(result){
			...
			$.post('./sct_processor.php', {'uname':username, 'action' : 'editSingle', 'index' : selectedIndex, 'data' : newNews },
				function(result){
					console.log("edit single : index " + selectedIndex + " : result : " + result);
					fetchDataAndPopulate('../../getScrolllist.php');
			});
		}
		else if(inputMode == "changeSeperator"){
//			newNews = escapeHtml(newNews);
			$.post('./sct_processor.php', {'uname' : username, 'action' : 'changeSeperator', 'data' : newNews }, function(result){
				console.log("change seperator new seperator : " + newNews + " result : " + result);
				fetchDataAndPopulate('../../getScrolllist.php');
			});
		}
...
function block(index){
	$.post('./sct_processor.php', {'uname' : username, 'action' : 'blockSingle', 'index' : index }, function(result){
...
function remove(index){
	...
	if(confirmation){
		$.post('./sct_processor.php', {'uname' : username, 'action':'removeSingle', 'index' : index }, function(result){
			fetchDataAndPopulate('../../getScrolllist.php');
		});
	}
}
...
console.log("pop : " + texts.length);
	...
	$("#seperator_control_value").text(seperator);
	$("#seperator_control_representation").html(insertHtml(seperator));
}
...
//			console.log(url+" successfully fetched. result : " + result);
			if(result != "none"){
				...
				seperator = xml.getElementsByTagName("root")[0].getAttribute('seperator');
